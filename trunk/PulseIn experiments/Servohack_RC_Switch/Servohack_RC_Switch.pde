...
#define SERV0_NETRUAL 1525	//uS value of netrual position
#define DB_LOWER 1400		//lower limit of the dead band 1050
#define DB_UPPER 1600		//upper limit of the dead band 1950
...
void setup ()
{
	...
	Serial.begin(9600);
	Serial.println("RC Servo to H-Bridge Motor Hack");	//Let us know what your doin'
...
void loop () 
{
	...
	usec = map(pot, 0, 1023, SERVO_REVERSE, SERVO_FORWARD);	//map it full range mode - repeat until its time
	//usec = map(pot, 0, 1023, DB_LOWER, DB_UPPER);			//map it deadband mode- repeat until its time
