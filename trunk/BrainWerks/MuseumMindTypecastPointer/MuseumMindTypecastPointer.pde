...
#define SHIFTDATA 9	// 8 port shift register pins (TPIC6A596NE)
#define SHIFTLATCH 7
#define SHIFTCLOCK 8
...
{												// creates and instance of buffer named cmd and a pointer to cmd named pcmd
	char adrbuf[3], cmdbuf[3], valbuf[13];
	int param;
}	cmd;
void* pcmd = &cmd;							
//buffer* pcmd = &cmd;							//broken because of arduino IDE auto prototying
...
void printaddr(char x);			//function prototypes
//void setbaud(char Mybaud);
void readserial();
void runcommand(void* pbuf);	//has to be void because of arduino IDE stupid auto prototying
...
//		Serial.println(Rxptr);
		...
		cmd.adrbuf[0] = Rxbuf[0];
		cmd.adrbuf[1] = Rxbuf[1];
		cmd.adrbuf[2] = 0; //null terminate
//		Serial.print("rxaddress: ");
//		Serial.println(rxaddress, DEC);
		cmd.cmdbuf[0] = toupper(Rxbuf[2]); //copy and convert to upper case
		cmd.cmdbuf[1] = toupper(Rxbuf[3]);
		cmd.cmdbuf[2] = 0;
//		Serial.print("cmdbuf: ");
//		Serial.println(cmd.cmdbuf);
//		cmd.valbuf[0] = Rxbuf[4];
		for (int i = 4 ; i <= Rxptr ; i++)
		{
			cmd.valbuf[i-4] = toupper(Rxbuf[i]);
//			Serial.println(cmd.valbuf[i-4], DEC);
		}
		cmd.valbuf[Rxptr-3] = 0;
		cmd.param = atoi(cmd.valbuf);
//		putstring("valbuf:");
//		Serial.println(cmd.valbuf);
//		Serial.print("parameter: ");
//		Serial.println(cmd.param, DEC); //
	}
}//we now have the address in rxaddress, the two letter command code in cmdbuf and the parameter value in cmd.param, We are ready to execute

void runcommand(void * pbuf)		//execute the command passed in buf
{                                       //Note: any changes in values in buf are not perminate, a copy is passed to runcommand and disappears when the command is complete
	// set typecast pointer-to-void to a pointer-to-buffer,			Adurino is not C!  I should be able to pass a pointer-to-buffer, pbuf*/
	buffer * pointer;
	pointer = (buffer *)pbuf;
	if ((atoi(pointer->adrbuf) == Unitaddress) || ((pointer->adrbuf[0] == '*') && (pointer->adrbuf[1] == '*')) ) //** is wildcard address, all units respond
	{
//		putstring_nl("is my pointer working?");
//		Serial.println(pointer->cmdbuf);
		
		if (strcmp(pointer->cmdbuf,"ON")==0)                        			//turn relay param ON
		{
			//'Print "command was ON"
			if ((pointer->param <= relaycount) && (pointer->param >= 0)) 
			{
				if (pointer->valbuf[0] =='*')							//wildcard
				{
					relaystate = 0xFF;
				...
				{
					relaystate = ((1<<(pointer->param-1)) | relaystate);  // turn bit pointer->param-1 on
					//digitalWrite(SHIFTLATCH, HIGH);
					shiftOut(relaystate);
					//digitalWrite(SHIFTLATCH, LOW);
					//Serial.print("relaystate: ");
		...
		if (strcmp(pointer->cmdbuf,"OF")==0)                          		//turn relay pointer->param OFF
		{
			...
			if ((pointer->param <= relaycount) && (pointer->param >= 0)) 
			{
				...
				{		
					relaystate = (~(1<<(pointer->param-1)) & relaystate);		//turn bit pointer->param-1 off
					//digitalWrite(SHIFTLATCH, LOW);
					shiftOut(relaystate);
					//digitalWrite(SHIFTLATCH, HIGH);
				}
		...
		if (strcmp(pointer->cmdbuf,"RW")==0)                       			//Write pointer->param to the relay state
		{
//			putstring("valbuf: ");
//			Serial.print(pointer->valbuf);
			if (pointer->valbuf[0] == '*')
			{
				relaystate = 0xFF;
				shiftOut(relaystate);
			}
			else if ((pointer->param <= 255) && (pointer->param >= 0)) 
			{
//				putstring("param: ");
//				Serial.print(pointer->param);
				relaystate = pointer->param;
				shiftOut(relaystate);
			} else {
				//Serial.print(pointer->param);
		...
		if (strcmp(pointer->cmdbuf,"RS")==0)                           		// Relay Status
		{
			...
			//digitalWrite(SHIFTLATCH, HIGH);
			printaddr(2);
			if ((pointer->param > 0) && (pointer->param <= relaycount))
			{				
				Serial.println((1<<pointer->param-1) == ((1<<pointer->param-1) & relaystate));
			}
			else 								//wildcard
			{
				Serial.println(relaystate, BIN);
		...
		if (strcmp(pointer->cmdbuf,"RT")==0)									//	Relay Test routine
		{
		printaddr(2);
			do
			{
				...
				putstring_nl("");
				readserial();
//				Serial.print(pointer->cmdbuf);
			} while ((strcmp(pointer->cmdbuf,"RT")==0) &&(pointer->valbuf[0]=='*'));
		...
		if (strcmp(pointer->cmdbuf,"IS")==0)                            		// Input Status
		{
			printaddr(2);
			if (pointer->valbuf[0] =='*')
			{
			...
			else if (pointer->param <= 16 && pointer->param > 0)
			{
/*				Serial.print("param: ");
				Serial.print(pointer->param);
				int mux0 = (1 == (B0001 & (pointer->param-1)));
				int mux1 = (2 == (B0010 & (pointer->param-1)));
				int mux2 = (4 == (B0100 & (pointer->param-1)));
				int mux3 = (8 == (B1000 & (pointer->param-1)));
				...
				Serial.print(" ");							*/
				digitalWrite(MUXS0, (1 == (1 & pointer->param-1)));
				digitalWrite(MUXS1, (2 == (2 & pointer->param-1)));
				digitalWrite(MUXS2, (4 == (4 & pointer->param-1)));
				digitalWrite(MUXS3, (8 == (8 & pointer->param-1)));
				analogin[pointer->param-1] = analogRead(MUXSIG);
				Serial.println(analogin[pointer->param-1]);
			}
			else
			{
			//Serial.println("param out of range");
		...
		if (strcmp(pointer->cmdbuf,"SA")==0)                            		// Set Address and save to EEP
		{
			putstring_nl("command was Set Address");
			if ((pointer->param >= 0) && (pointer->param <= 99))
			{
				Unitaddress = pointer->param;   //make it the address
...
/*		if (strcmp(pointer->cmdbuf,"SB")==0)                              	// Set Baud and save to EEP              {
		{      
			//'Print "command was Set Baud"
			if ((pointer->param > 0) && (pointer->param <= 10))
			{
				Unitbaud = pointer->param;   
		...
		if (strcmp(pointer->cmdbuf,"PS")==0)									//Play a Sound, (string) pointer->valbuf is a filename in 8.3
		{																//pointer->param is garbage
//			putstring("valbuf: ");
//			Serial.print(pointer->valbuf);
			if (pointer->valbuf[0] =='*')
			{
//				putstring_nl("Play All");
			...
			{
//				putstring("Play: ");
//				Serial.println(pointer->valbuf);
				playfile(pointer->valbuf);
				root.rewind();
			}
		printaddr(2);
//		putstring_nl("Done playing");
		...
		if (strcmp(pointer->cmdbuf,"LS")==0)									//Lists All files recursively
		{
			printaddr(2);
...
//			root.rewind();
		...
		if (pointer->cmdbuf[0]=='A')											//enters pointer->param as the Array state at row pointer->cmdbuf[1]
		{
			putstring("Command is A");
			array[pointer->cmdbuf[1]-48] = pointer->param;
			Serial.println(pointer->cmdbuf[1]-48);
			for (int i = 0; i < 10; i++ )
			{
