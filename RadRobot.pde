...
unsigned int _time[] = {0,0,0};				// shadow copy for new values

#define FIRE 19  //aka A5		Output pins
...
/*		Serial.print("\t");
	...
	for(byte i=0; i < PIN_COUNT; i++)
	{
		if ((1000<=_time[i])&&(_time[i] <= 2024))  //check bounds
		{
			time[i]=_time[i]-1000; //time[i] is between 0-1024
		}
	}
	
	digitalWrite(FIRE,  time[2]>500 ? HIGH : LOW); //set fire first;
	time[2]=time[1]+time[0];		//left leg, values between 0-2048
	time[1]=time[1]-time[0];		//right leg
...
{
	state=FALLING;
	_time[i]=Timer1.read();	// Needs Timer1-v2
	Timer1.stop();
//	Serial.print('f');
}
